...
?>
<div class="box clearfix box-with-products <?php if($theme_options->get( 'product_scroll_latest' ) != '0') { echo 'with-scroll';  } ?> box-no-advanced">
  <?php if($theme_options->get( 'product_scroll_latest' ) != '0') { ?>
  <!-- Carousel nav -->
    	...
    	<div id="myCarousel<?php echo $id; ?>" <?php if($theme_options->get( 'product_scroll_latest' ) != '0') { ?>class="carousel slide"<?php } ?>>
    		...
    		<div class="carousel-inner">
    			<?php $i = 0; $row_fluid = 0; $item = 0; foreach ($products as $product) { $row_fluid++; ?>
	    			...
	    			<?php $r=$row_fluid-floor($row_fluid/$all)*$all; if($row_fluid>$all && $r == 1) { if($theme_options->get( 'product_scroll_latest' ) != '0') { echo '</div></div></div><div class="item"><div class="product-grid"><div class="row">'; $item++; } else { echo '</div><div class="row">'; } } else { $r=$row_fluid-floor($row_fluid/$row)*$row; if($row_fluid>$row && $r == 1) { echo '</div><div class="row">'; } } ?>
