...
<head>
    ...
    if($theme_options->get( 'display_specials_countdown' ) == '1') {
         ...
         if(isset($countdown[$language_id])) {
         	...
         	$lista_plikow[] = 'catalog/view/theme/'.$config->get($config->get('config_theme') . '_directory').'/js/countdown/jquery.countdown.min.js';
         }
    }
    ...
    <?php if($theme_options->get( 'quick_search_autosuggest' ) != '0' && $_GET['route'] != 'affiliate/tracking' && $class != 'account-tracking') { ?>
