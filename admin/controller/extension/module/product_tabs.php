...
class ControllerExtensionModuleProductTabs extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if(isset($this->request->post['product_tabs'])) {
				if(is_writable(DIR_APPLICATION . 'controller/extension/module/')) {
					...
					$this->session->data['error_warning'] = 'You need to set CHMOD 777 for all folder and subfolder in admin/controller/extension/module/!';
				}
			} else {
			...
			$this->response->redirect($this->url->link('extension/module/product_tabs', 'token=' . $this->session->data['token'], true));			
		}
		...
		$data['action'] = $this->url->link('extension/module/product_tabs', 'token=' . $this->session->data['token'], true);
