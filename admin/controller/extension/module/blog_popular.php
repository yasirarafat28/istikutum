...
class ControllerExtensionModuleBlogPopular extends Controller {
	...
	public function index() {   
		$this->language->load('extension/module/blog_popular');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('extension/module/blog_popular', 'token=' . $this->session->data['token'], true));
		}
		...
		$data['action'] = $this->url->link('extension/module/blog_popular', 'token=' . $this->session->data['token'], true);
		...
		foreach ($directories as $directory) {
			...
			$files_popular_list = glob(DIR_CATALOG . 'view/theme/' . $directory . '/template/extension/module/blog_popular/*.tpl');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/blog_popular', 'token=' . $this->session->data['token'], true)
		);
		...
		$this->response->setOutput($this->load->view('extension/module/blog_popular', $data));
	}
	
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/blog_popular')) {
